...
//$.coral.datepicker.initialized = false;
...
/*		setDefaults: function(options) {
	...
	_initElement: function(){
		...
		this.element.addClass("coral-textbox-default coral-validation-datepicker");
	...
	_processDefaultDate : function(that,defaultDate){
		...
		try{
			defaultDateObj=that.parseDate(dateFormat,defaultDate,that._getFormatConfig());
		}catch(e){
...
//			that._apply(inst,"onFormatWarn",[defaultDate,inst]);
		...
		var date=that._daylightSavingAdjustWidthTime(new Date(opts.currentYear, opts.currentMonth, opts.currentDay),[opts.currentHur,opts.currentMiu,opts.currentSed]);
		var datestr = "";
		if ( !opts.value && defaultDate ) {
			opts.value = defaultDate;
		}
		if ( opts.value != "" ) {
			var srcDateFormat = this.getFormatter(opts.value,opts.dateFormat, this._getFormatConfig());
			datestr = this.completeFormate(srcDateFormat,dateFormat);
			datestr = this.formatDate(datestr, date, this._getFormatConfig());
		} 
	...
	_setOption: function(key, value) {
		...
		if( key=="value" ){
			if(value){
				var format = this.getFormatter(value,opts.dateFormat, this._getFormatConfig());
				var cdate = this.parseDate(format, value);
				var srcDateFormat = opts.srcDateFormat;
				if (this._checkDateRange(cdate) === 2){
					value = this.formatDate(srcDateFormat, minDate, this._getFormatConfig());
				}
				if (this._checkDateRange(cdate) === 1) {
					value = this.formatDate(srcDateFormat, maxDate, this._getFormatConfig());
				}
			}
			date = that._processDefaultDate(that,value);
			//设置隐藏字段值
		}
		if (minDate !== null && key=="dateFormat") {
			opts.minDate = this._formatDate(minDate);
		}
		if (maxDate !== null && key == "dateFormat") {
	...
	_doKeyUp: function(event) {
		...
		if (opts.input.val() !== opts.lastVal) {
			try {
				...
				date = that.parseDate(opts.dateFormat,
					opts.input ? opts.input.val() : null),
					that._getFormatConfig();
				// 标识是用户在键盘输入的编辑状态
...
/*					that._apply(inst,"onFormatError",[dateStr,inst]);
	...
	_setHiddenInputValue: function() {
		...
		var date=this._daylightSavingAdjustWidthTime(new Date(opts.currentYear, opts.currentMonth, opts.currentDay),[opts.currentHur,opts.currentMiu,opts.currentSed]);
		var format = this.getFormatter(opts.input.val(),opts.dateFormat, this._getFormatConfig()),
			datestr = this.completeFormate(format,dateFormat);
		if(opts.complete){
			dateStr=this.formatDate(srcDateFormat, date, this._getFormatConfig());
		}else {
			...
			if(srcDateFormat.indexOf("-")>-1 ||srcDateFormat.indexOf("/")>-1 ){
				...
				var dateStr = this.formatDate(datestr, date, this._getFormatConfig());
	...
	_inputBlur : function(e){
		...
		if ( (typeof opts.currentYear != "undefined" || opts.currentYear != null) && !opts.isShow ) {
			var srcDateFormat = this.getFormatter(opts.input.val(),opts.dateFormat, this._getFormatConfig());
			// 修改赋值bug 20150129
			if ( "" != opts.input.val() ) {
				datestr = this.completeFormate(srcDateFormat,dateFormat);
				datestr = this.formatDate(datestr, date, this._getFormatConfig());
	...
	focus: function(event) {
		var input = event.target || this.elment[0],
		    opts = this.options;
	...
	_gotoToday: function(id) {
		var that = this,
			...
			target = $(id),
			opts = this.options;
		var hasTime = this._hasTime(opts.dateFormat);
...
//			options.inst && (options.inst.beforevalue = vValue);
		format = this.getFormatter( vValue[0], vformat[0], options);
	...
	parseDate: function (format, value, options) {
		...
		fValue = this.configDateFormat(format,value,options);
		...
		var iFormat, dim, extra,
			iValue = 0,
			shortYearCutoffTemp = (options ? options.shortYearCutoff : null) || this.options.shortYearCutoff,
			shortYearCutoff = (typeof shortYearCutoffTemp !== "string" ? shortYearCutoffTemp :
				new Date().getFullYear() % 100 + parseInt(shortYearCutoffTemp, 10)),
			dayNamesShort = (options ? options.dayNamesShort : null) || this.options.dayNamesShort,
			dayNames = (options ? options.dayNames : null) || this.options.dayNames,
			monthNamesShort = (options ? options.monthNamesShort : null) || this.options.monthNamesShort,
			monthNames = (options ? options.monthNames : null) || this.options.monthNames,
			opts=options.opts,
			year = -1,
			month = -1,
			day = -1,
...
W W	同上，如果小于两位数，前面补零 (01 - 53) 。
	...
	_setDateFromField: function( noDefault) {
		...
		var dateFormat = opts.dateFormat,
			...
			options = this._getFormatConfig();
		
		try {
			date = this.parseDate(dateFormat, dateValue, options) || defaultDate;
		} catch (event) {
		...
		var offsetNumeric = function(offset) {
			...
			offsetString = function(offset) {
				try {
					...
					return that.parseDate(tmpFormat,
						offset, that._getFormatConfig());
				}catch (e) {
					// Ignore
					//return "Invalid Date";
					//触发格式化错误事件
/*						that._apply(inst,"onFormatError",[date,inst]);TODO: 涉及_apply的以后要重写
		...
		if (opts.input) {
			if ( clear ) {
				...
				if(opts.complete){
					...
					var format = this.getFormatter(opts.value,opts.dateFormat, this._getFormatConfig());
	...
	_getDate: function() {
		var opts = this.options,
			...
			this._daylightSavingAdjustWidthTime( new Date( opts.currentYear, opts.currentMonth, opts.currentDay ), [opts.currentHur,opts.currentMiu,opts.currentSed] ) );
		return startDate;
	},
...
//					var opts = this.options;
	...
	_generateHTML: function() {
		var opts = this.options,
			...
			currentDate = this._daylightSavingAdjustWidthTime((!opts.currentDay ? new Date(9999,9,9) :
		...
		if (maxDate) {
			...
			while (this._daylightSavingAdjustWidthTime(new Date(drawYear, drawMonth, 1),[opts.currentHur,opts.currentMiu,opts.currentSed]) > maxDraw) {
				drawMonth--;
				if (drawMonth < 0) {
					drawMonth = 11;
					drawYear--;
		...
		prevText = (!navigationAsDateFormat ? prevText : this.formatDate(prevText,
			this._daylightSavingAdjustWidthTime(new Date(drawYear, drawMonth - stepMonths, 1),[opts.currentHur,opts.currentMiu,opts.currentSed]),
		...
		for (row = 0; row < numMonths[0]; row++) {
			...
			for (col = 0; col < numMonths[1]; col++) {
				selectedDate = this._daylightSavingAdjustWidthTime(new Date(drawYear, drawMonth, opts.selectedDay),[opts.currentHur,opts.currentMiu,opts.currentSed]);
				...
				if (hasTime) {
					 ...
					 calender +="<a id='timePanel' class='date-time-icon time-btn'><span class='cui-icon-clock'/></a>" +
					 	"<a class='datepicker-close time-btn' data-handler='close' data-event='mousedown'>关闭</a></div>";
				 }
	...
	_adjustInstDate: function(offset, period) {
		...
		var year = opts.drawYear + (period === "Y" ? offset : 0),
			...
			day = Math.min(opts.selectedDay, this._getDaysInMonth(year, month)) + (period === "D" ? offset : 0),
			date = this._restrictMinMax(this._daylightSavingAdjustWidthTime(new Date(year, month, day),[opts.currentHur,opts.currentMiu,opts.currentSed]));
	...
	_getDaysInMonth: function(year, month) {
		...
		var opts = this.options;
		if ( opts ) {
			return 32 - this._daylightSavingAdjustWidthTime(new Date(year, month, 32),[opts.currentHur,opts.currentMiu,opts.currentSed]).getDate();
		} else {
	...
	_formatDate: function(day, month, year) {
		...
		var date = (day ? (typeof day === "object" ? day :
			this._daylightSavingAdjustWidthTime(new Date(year, month, day),[opts.currentHur,opts.currentMiu,opts.currentSed])) :
			this._daylightSavingAdjustWidthTime(new Date(opts.currentYear, opts.currentMonth, opts.currentDay),[opts.currentHur,opts.currentMiu,opts.currentSed]));
		date.setHours(opts.currentHur);
		date.setMinutes(opts.currentMiu);
		date.setSeconds(opts.currentSed);
		date.setMilliseconds(0);
		return this.formatDate(opts.dateFormat, date, this._getFormatConfig());
	},
	//调用事件
