	...
	$.component("coral.swfuploader", {
		...
		_create: function() {
			...
			if (flashInstalled) {
				...
				window['uploadify_' + opts.id] = new SWFUpload(swfUploadSettings);
				var swfuploaderObj = window['uploadify_' + opts.id];

				// Add the SWFUpload object to the elements data object
				this.element.data('uploadify', swfuploaderObj);
				// Wrap the instance
				...
				$wrapper.data('uploadify', swfuploaderObj);

				/*// Create the button
				var $button = $('<div />', {
					'id'    : opts.id + '-button',
					'class' : 'uploadify-button ' + opts.buttonClass
				});
				if (opts.buttonImage) {
					$button.css({
				...
				$button.html('<span class="uploadify-button-text">' + opts.buttonText + '</span>')
				.css({
					'height'      : opts.height + 'px',
		...
		cancel: function(fileID, supressEvent) {
			var args = arguments,uploadList,downList,
				that = this;
			// Create a reference to the jQuery DOM object
			var swfuploaderObj = this.element.data('uploadify'),
			...
			if ($('#' + fileID).hasClass("template-download")) {
				...
				settings.uploadedFileList = [];
				settings.unUploadedFileList = [];
			}
		},
		...
		_destroy: function() {
			...
			this.element.find(".uploadify").remove();
			// Destroy the queue
        ...
        _setOption: function(key, value, resetObjects) {
			...
			switch (key) {
				...
				case 'buttonCursor':
					...
					break;
				case 'buttonText':
					$('#' + this.options.id + '-button').find('.uploadify-button-text').html(value);
					break;
				case 'width':
		...
		onSelect: function(swfObj, file) {
			...
			var errorMsg = this.validateFiles(swfObj,file);
			...
			$("#" + file.id).find('.error').text(errorMsg);
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						swfObj.queueData.errorMsg = 'The File is too large';
						break;
		...
		onUploadError : function(swfObj, file, errorCode, errorMsg) {
			...
			if ($.inArray('onUploadError', settings.overrideEvents) < 0) {
				...
				/*if (errorCode != SWFUpload.UPLOAD_ERROR.FILE_CANCELLED && errorCode != SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED) {
					$('#' + settings.unUploadedFileList[0].fileId).addClass('uploadify-error');
				}
		...
		onUploadProgress : function(swfObj, file, fileBytesLoaded, fileTotalBytes) {
			...
			if ($.inArray('onUploadProgress', settings.overrideEvents) < 0) {
				/*if (settings.progressData == 'percentage') {
					$('#' + file.id).find('.data').html(' - ' + progress + '%');
				} else if (settings.progressData == 'speed' && lapsedTime > 500) {
					$('#' + file.id).find('.data').html(' - ' + swfObj.queueData.averageSpeed + suffix);
				}*/
				//$('#' + file.id).find('.uploadify-progress-bar').css('width', percentage + '%');
				$('#' + file.id).find('.progress').progressbar("value", progress);
			}
